...
function displayTables() {
  ...
  const servingTables = getReadyTables();
  const serving = servingTables.map((t) => {
    return `<div class='table'>` + t + `</div>`;
  });
  tables.innerHTML = serving;
  tables.innerHTML = tables.innerHTML.replaceAll(',', '');
